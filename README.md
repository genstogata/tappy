...
5. **View the report** — click **Report** to see each student's number of times out, current status, and total time out for the day. Click **Details** on a row to see the local clock time of each tap-out/tap-in (also shown in the exported CSV). The report header shows the device's current local time so you can confirm the clock is correct. Use **Export CSV** or **Print** to save a copy — this is important since data isn't synced between devices.
